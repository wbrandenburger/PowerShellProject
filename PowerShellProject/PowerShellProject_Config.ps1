...
$config_content = Format-IniContent -Content $config_content -Substitution $Module 

# read module settings from module format file
$config_format = Get-Content -Path $Module.ConfigFormat | ConvertFrom-Json

Format-JsonContent -Content $config_format -Substitution $Module | ForEach-Object{
    ...
    if ($config_content.Keys -match $_.Section){
        ...
        if ($sec_keys -match $_.Name){
            ...
            if ($_.Id -in $Module.PSObject.Properties.Name) {
                $Module.($_.Id) = $value
            } else{
                $Module | Add-Member -MemberType NoteProperty -Name $_.Id -Value $value
            }
...
Write-FormattedMessage -Message "Module config file: $($Module.Config)" -Module $Module.Name -Color DarkYellow

Write-FormattedMessage -Message "Working directory: $($Module.WorkDir)" -Module $Module.Name -Color DarkYellow